...
def _render_project_card(project: Project, parent_container):
    with ui.card().classes("w-full cursor-pointer hover:shadow-lg transition-shadow"):
        with ui.card_section():
            ...
            ui.label(f"Year: {project.year}").classes("text-sm text-gray-600")
        ...
        with ui.card_actions().classes("justify-end"):
            ui.button("Open", on_click=lambda p=project: ui.navigate.to(f"/project/{p.id}/wall")).props("flat color=primary")
...
def _show_create_dialog(cards_container):
    with ui.dialog() as dialog, ui.card().classes("w-96"):
        ...
        with ui.row().classes("justify-end w-full mt-4"):
            ui.button("Cancel", on_click=dialog.close).props("flat")
            ui.button("Create", on_click=lambda: _do_create(name_input.value, int(year_input.value), dialog, cards_container)).props("color=primary")
    dialog.open()


def _do_create(name: str, year: int, dialog, cards_container):
    if not name or not name.strip():
        ui.notify("Please enter a project name", type="warning")
        return
    db = get_db()
    try:
        project = Project(id=generate_id(), name=name.strip(), year=year)
        db.add(project)
...
def _import_project_json(content: bytes, cards_container):
    ...
    try:
        pid = generate_id()
        project = Project(id=pid, name=name, year=year)
        db.add(project)
